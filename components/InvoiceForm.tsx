...
import type { Invoice, LineItem, Client, Product, Settings, InvoiceTemplate } from '../types';
...
// Helper function to calculate totals, moved outside for reuse within the reducer
const calculateAllTotals = (items: LineItem[], taxRate: number, amountPaid: number) => {
  ...
  return { subtotal, taxAmount, total, costSubtotal, netProfit, balanceDue };
};


type FormState = Omit<Invoice, 'id'> & { customItemFlags: Record<string, boolean> };

type Action =
  | { type: 'SET_INVOICE'; payload: { invoice: Omit<Invoice, 'id'>, flags: Record<string, boolean>} }
  | { type: 'CHANGE_FIELD'; payload: { field: keyof Omit<Invoice, 'id'>; value: any } }
  | { type: 'ADD_ITEM' }
...
const invoiceReducer = (state: FormState, action: Action): FormState => {
  switch (action.type) {
    case 'SET_INVOICE':
    ...
    case 'CHANGE_FIELD': {
      ...
      if (action.payload.field === 'taxRate') {
        const totals = calculateAllTotals(newState.items, newState.taxRate, newState.amountPaid);
    ...
    case 'ADD_ITEM': {
      ...
      const totals = calculateAllTotals(newItems, state.taxRate, state.amountPaid);
...
const AUTOSAVE_DEBOUNCE_TIME = 1500; // ms
const getAutoSaveKey = (invoiceId?: number) => 
  invoiceId ? `autosavedInvoice_edit_${invoiceId}` : 'autosavedInvoice_new';
...
const InvoiceForm: React.FC<InvoiceFormProps> = ({ id, onSave, onSaveProduct, onCancel, clients, products, settings }) => {
  const { t } = useI18n();
  const getInitialInvoiceState = useCallback((settings: Settings | null): FormState => {
    ...
    return {
      invoiceNumber: '', status: InvoiceStatus.Draft,
      ...
      payments: [], amountPaid: 0, balanceDue: 0,
      createdAt: now,
      updatedAt: now,
  ...
  const [state, dispatch] = useReducer(invoiceReducer, getInitialInvoiceState(settings));
  ...
  const generateInvoiceNumber = useCallback(async () => {
    ...
    const count = id ? await db.invoices.count() : (await db.invoices.count()) + 1;
    const date = new Date();
  ...
  }, [settings?.invoiceNumberFormat, id]);
  ...
  useEffect(() => {
    const loadInvoice = async () => {
      ...
      if (id) {
        ...
        if (existingInvoice) {
            dispatch({ type: 'SET_INVOICE', payload: { invoice: existingInvoice, flags: {} } });
        }
      } else {
        const newInvoiceState = getInitialInvoiceState(settings);
        newInvoiceState.invoiceNumber = await generateInvoiceNumber();
        dispatch({ type: 'SET_INVOICE', payload: { invoice: newInvoiceState, flags: newInvoiceState.customItemFlags } });
      }
      setIsLoading(false);
    };
    loadInvoice();
  }, [id, settings, generateInvoiceNumber, getInitialInvoiceState]);
  ...
  useEffect(() => {
    ...
    autoSaveTimeoutRef.current = window.setTimeout(() => {
        const isPristine = !invoice.toName && invoice.items.length === 1 && !invoice.items[0].name && invoice.items[0].price === 0;
        if (!isPristine) {
            const autoSaveKey = getAutoSaveKey(id);
...
}, [state, invoice, id, isLoading, autoSavedData]);
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    // FIX: Cast name to keyof Invoice to resolve type error.
    dispatch({ type: 'CHANGE_FIELD', payload: { field: name as keyof Omit<Invoice, 'id'>, value: parsedValue } });
  };
  ...
  const handleSubmit = () => {
    ...
    const newCustomItems = invoice.items.filter(item => 
      item.name.trim() !== '' &&
  ...
  const handleRestoreConfirm = () => {
    if (autoSavedData) {
      dispatch({ type: 'SET_INVOICE', payload: { invoice: autoSavedData, flags: autoSavedData.customItemFlags }});
    ...
    <>
      <div className="bg-white shadow-lg rounded-xl p-4 sm:p-6 lg:p-8 space-y-8 max-w-6xl mx-auto">
        <div className="flex justify-between items-center">
            <h2 className="text-2xl font-bold text-gray-800">{id ? t('editInvoice') : t('createInvoice')} #{invoice.invoiceNumber}</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <section aria-labelledby="to-heading">
            ...
            <div className="space-y-2">
              ...
              <input id="toName" name="toName" value={invoice.toName} onChange={handleChange} placeholder={t('clientsName')} className="w-full p-2 border rounded"/>
              <label htmlFor="toEmail" className="sr-only">{t('clientsEmail')}</label>
              <input id="toEmail" name="toEmail" type="email" value={invoice.toEmail} onChange={handleChange} placeholder={t('clientsEmail')} className="w-full p-2 border rounded"/>
            </div>
          </section>
        </div>
        <section aria-labelledby="dates-heading" className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div>
              ...
              <input type="date" id="invoiceDate" name="invoiceDate" value={invoice.invoiceDate} onChange={handleChange} className="mt-1 w-full p-2 border rounded"/>
          </div>
          <div>
              <label htmlFor="dueDate" className="block text-sm font-medium text-gray-700">{t('dueDate')}</label>
              <input type="date" id="dueDate" name="dueDate" value={invoice.dueDate} onChange={handleChange} className="mt-1 w-full p-2 border rounded"/>
          </div>
        </section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-4 border-t">
            <section aria-labelledby="notes-heading">
              ...
              <textarea aria-labelledby="notes-heading" name="notes" value={invoice.notes} onChange={handleChange} rows={4} className="w-full p-2 border rounded"></textarea>
            </section>
            <section role="region" aria-live="polite" aria-label={t('invoiceTotals')} className="space-y-2 bg-gray-50 p-4 rounded-lg">
               ...
               <div className="flex justify-between items-center text-red-600"><span className="text-gray-600">{t('costSubtotal')}</span><span className="font-medium">{formatCurrency(invoice.costSubtotal, settings, invoice.currency)}</span></div>
               <div className="flex justify-between items-center">
                  <div className="flex items-center gap-2">
                    <label htmlFor="taxRate" className="text-gray-600">{t('tax')}</label>
                    <input type="number" id="taxRate" name="taxRate" value={invoice.taxRate} onChange={handleChange} className="w-20 p-1 border rounded text-right" />
                  </div>
                  <span className="font-medium">{formatCurrency(invoice.taxAmount, settings, invoice.currency)}</span>
               </div>
               <div className="flex justify-between items-center font-semibold text-green-700 border-t pt-2 mt-2"><span className="text-gray-600">{t('netProfit')}</span><span>{formatCurrency(invoice.netProfit, settings, invoice.currency)}</span></div>
        ...
        <div className="flex justify-end gap-4 pt-4 border-t">
          ...
          <button onClick={handleSubmit} className="px-6 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 shadow-md">{id ? t('updateInvoice') : t('saveInvoice')}</button>
        </div>
      </div>
      {autoSavedData && (
        <ConfirmDialog
